...
class ClockSpeedsApp:
    ...
    def create_more_button(self):
        ...
        try:
            more_button = widget_factory.create_button(self.main_box, "...", self.show_more_options, margin_start=10, margin_end=10)
        except Exception as e:
    ...
    def show_more_options(self, widget):
        ...
        try:
            ...
            settings_button = widget_factory.create_button(more_box, "Settings", settings_window.open_settings_window, margin_start=15, margin_end=15)
            system_button = widget_factory.create_button(more_box, "System", self.open_system_window, margin_start=15, margin_end=15)
            about_button = widget_factory.create_button(more_box, "About", self.open_about_dialog, margin_start=15, margin_end=15)
    ...
    def open_system_window(self, widget=None):
        ...
        try:
            ...
            widget_factory.create_label(system_fixed, markup="<b>System Information</b>", x=10, y=10)

            y_offset = 40
            for key, value in cpu_info.items():
                if key not in ["Min (MHz)", "Max (MHz)", "Physical Cores", "Virtual Cores (Threads)", "Total RAM (MB)", "Cache Sizes"]:
                    widget_factory.create_label(system_fixed, text=f"{key}: {value}", x=10, y=y_offset)
                    y_offset += 30

            if "Cache Sizes" in cpu_info:
                cache_sizes = cpu_info["Cache Sizes"]
                widget_factory.create_label(system_fixed, text="Cache Sizes:", x=10, y=y_offset)
                y_offset += 30
                for cache_type, size in cache_sizes.items():
                    widget_factory.create_label(system_fixed, text=f"{cache_type}: {size}", x=10, y=y_offset)
                    y_offset += 30

            widget_factory.create_label(system_fixed, text=f"Cores: {cpu_info['Physical Cores']}  Virtual Cores (Threads): {cpu_info['Virtual Cores (Threads)']}", x=10, y=y_offset)
            y_offset += 30

            widget_factory.create_label(system_fixed, text=f"Total RAM (MB): {cpu_info['Total RAM (MB)']}", x=10, y=y_offset)
            y_offset += 30
            ...
            if min_frequencies and max_frequencies:
                widget_factory.create_label(system_fixed, markup="<b>Allowed CPU Frequencies</b>", x=10, y=y_offset)
                y_offset += 30
                ...
                for (min_freq, max_freq), threads in thread_groups.items():
                    ...
                    widget_factory.create_label(system_fixed, text=f"{', '.join(thread_ranges)}: {min_freq:.2f} MHz - {max_freq:.2f} MHz", x=10, y=y_offset)
                    y_offset += 30
    ...
    def open_about_dialog(self, widget=None):
        ...
        try:
            ...
            # About Tab
            about_tab = widget_factory.create_about_tab(about_notebook, "About")
            ...
            # Credits Tab
            credits_tab = widget_factory.create_about_tab(about_notebook, "Credits")
    ...
    def create_monitor_widgets(self):
        ...
        try:
            ...
            for i in range(cpu_file_search.thread_count):
                ...
                description_label = widget_factory.create_label(self.monitor_fixed, f"Thread {i}:", x=x_offset + 5, y=y_offset + 10)

                clock_label = widget_factory.create_entry(self.monitor_fixed, "N/A MHz", False, width_chars=10, x=x_offset + 80, y=y_offset + 5)

                progress_bar = widget_factory.create_cellrendererprogress(self.monitor_fixed, x=x_offset + 175, y=y_offset - 5)
            ...
            average_description_label = widget_factory.create_label(self.monitor_fixed, "Average:", x=16, y=y_offset + 4)

            self.average_clock_entry = widget_factory.create_entry(self.monitor_fixed, "N/A MHz", False, width_chars=10, x=80, y=y_offset)

            self.average_progress_bar = widget_factory.create_cellrendererprogress(self.monitor_fixed, x=175, y=y_offset - 15)

            package_temp_label = widget_factory.create_label(self.monitor_fixed, "Package Temp:", x=300, y=y_offset + 4)
    ...
    def create_control_widgets(self):
        ...
        try:
            ...
            for i in range(cpu_file_search.thread_count):
                ...
                cpu_min_max_checkbutton = widget_factory.create_checkbutton(self.control_fixed, f"Thread {i}", None, None, x=10, y=y_offset + 27)
                cpu_min_max_checkbutton.set_active(True)
                cpu_min_max_checkbutton.set_tooltip_text("Toggle Whether Min/Max Should Be Applied")

                cpu_max_scale = widget_factory.create_scale(self.control_fixed, scale_manager.update_min_max_labels, global_state.SCALE_MIN, global_state.SCALE_MAX, x=100, y=y_offset)
                cpu_max_scale.set_name(f'cpu_max_scale_{i}')
                cpu_max_scale.set_tooltip_text("Maximum Frequency Set in MHz")

                cpu_max_desc = widget_factory.create_label(self.control_fixed, f"Maximum", x=430, y=y_offset)

                cpu_min_scale = widget_factory.create_scale(self.control_fixed, scale_manager.update_min_max_labels, global_state.SCALE_MIN, global_state.SCALE_MAX, x=100, y=y_offset + 40)
                cpu_min_scale.set_name(f'cpu_min_scale_{i}')
                cpu_min_scale.set_tooltip_text("Minimum Frequency Set in MHz")

                cpu_min_desc = widget_factory.create_label(self.control_fixed, f"Minimum", x=430, y=y_offset + 40)
            ...
            tdp_label = widget_factory.create_label(self.control_fixed, "TDP:", x=50, y=y_offset + 160)
            self.tdp_scale = widget_factory.create_scale(self.control_fixed, None, global_state.TDP_SCALE_MIN, global_state.TDP_SCALE_MAX, x=100, y=y_offset + 150)
            self.tdp_scale.set_value(0)
            self.tdp_scale.set_tooltip_text("Adjust the TDP in Watts")

            tdp_installed = False
            if cpu_file_search.cpu_type == "Intel":
                apply_tdp_button = widget_factory.create_button(self.control_fixed, "Apply TDP", cpu_manager.set_intel_tdp, x=210, y=y_offset + 180)
                tdp_installed = True
            elif cpu_file_search.cpu_type == "Other" and ryzen_smu_installer.is_ryzen_smu_installed():
                apply_tdp_button = widget_factory.create_button(self.control_fixed, "Apply TDP", cpu_manager.set_ryzen_tdp, x=210, y=y_offset + 180)
                tdp_installed = True
